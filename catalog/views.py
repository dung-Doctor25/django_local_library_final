...
def index(request):
    ...
    num_books_has_muscel = Book.objects.filter(title__icontains='muscel').count()
    num_Science_Fiction_genres = Genre.objects.filter(name__icontains='Fiction').count()
    ...
    context = {
        ...
        'num_books_has_muscel': num_books_has_muscel,
        'num_Science_Fiction_genres': num_Science_Fiction_genres,
...
class LibrarianLoanedBooksByUserListView(PermissionRequiredMixin,generic.ListView):
    """Generic class-based view listing books on loan to each user."""
    model = BookInstance
    template_name = 'catalog/bookinstance_list_borrowed_user_librarian.html'
    paginate_by = 10
